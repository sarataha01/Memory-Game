...
//handle audio error
const safePlay = (audio: HTMLAudioElement, reset: boolean = true) => {
  try {
    if (reset) {
      audio.pause();
      audio.currentTime = 0;
...
const toggleFlip = (index: number) => {
  ...
  if (card.flip || !card.clicked) return;

  if (prepare.selectedIndex1 === index) return;
...
const updateProgress = () => {
  ...
  if (progressLabel) progressLabel.innerText = `${Math.round(progress)}%`;
};
